...
class PerformanceGUI:
    def __init__(self, animal_name="Animal"):
        ...
        self.ax_rttablehold = self.fig.add_subplot(gs[1])
        ...
        self.ax_rtdooropen = self.fig.add_subplot(gs[2])
    ...
    def update(self, results_df: pd.DataFrame, current_trial_port=None):
        ...
        # self.ax_rt.clear()
        # self.ax_rt.plot(trials, rts, c="black", alpha=0.7, linewidth=1)
        # self.ax_rt.scatter(trials, rts, c="black", s=20)
        ...
        if "rt" in results_df.columns:
            valid_c = results_df["rt"].notna()
            trials_c = results_df.loc[valid_c, "trial_num"]
            rts_c = results_df.loc[valid_c, "rt"]
        ...
        if "rt_tablehold" in results_df.columns:
            valid_table = results_df["rt_tablehold"].notna()
            trials_table = results_df.loc[valid_table, "trial_num"]
            rts_table = results_df.loc[valid_table, "rt_tablehold"]

            self.ax_rttablehold.clear()
            self.ax_rttablehold.grid(True)
            self.ax_rttablehold.plot(trials_table, rts_table, c="black", linewidth=1, alpha=0.8)
            self.ax_rttablehold.scatter(trials_table, rts_table, c="black", s=20)
        ...
        if "rt_dooropen" in results_df.columns:
            valid_door = results_df["rt_dooropen"].notna()
            trials_door = results_df.loc[valid_door, "trial_num"]
            rts_door = results_df.loc[valid_door, "rt_dooropen"]
            
            self.ax_rtdooropen.clear()
            self.ax_rtdooropen.grid(True)
            self.ax_rtdooropen.plot(trials_door, rts_door, c="black", linewidth=1, alpha=0.8)
            self.ax_rtdooropen.scatter(trials_door, rts_door, c="black", s=20)
...
class SensorGUI:
    def __init__(self):
        ...
        self.ax.text(0.5, 0.95, "Sensor C", ha="center", fontsize=9, transform=self.ax.transAxes)
        ...
        # Timestamp texts (under each label)
